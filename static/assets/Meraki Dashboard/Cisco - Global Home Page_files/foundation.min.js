...
}cdc.wcUtil=(function(){var c=function(f,d){var h=f.split("-");
for(var e=1;
e<h.length;
e++){h[e]=h[e].charAt(0).toUpperCase()+h[e].slice(1)
}h=h.join("");
if(window.hasOwnProperty(h)){return document.querySelector(f)[d]
}else{if(cdc.hasOwnProperty(h)){for(var g in cdc[h].methods){if(cdc[h].methods.hasOwnProperty(g)){if(cdc[h].methods[g].key===d){return cdc[h].methods[g].value
}}}}else{throw new TypeError("cdc.wcUtils cannot find "+d+" in "+f)
}}};
var a=function(d,f,g){var e,f=f||document;
e=f.querySelector("cdc-"+d+"-sat")||f.querySelector("cdc-"+d)||f;
if(!g){if(!!e.shadowRoot){e=e.shadowRoot
}e=e.querySelector("#fwt-"+d)||e.querySelector("#fw-"+d)||e
}return e
};
var b=function(i){var n=i.classList!==undefined,p,g,l,d,k=false;
if(n){var e=i.classList;
p=e.add!==undefined;
g=e.contains!==undefined;
l=e.remove!==undefined;
d=e.toggle!==undefined;
k=e.value!==undefined
}var o=function(q){if(p){e.add(q)
}else{i.className+=q
}};
var f=function(q){if(g){return e.contains(q)
}else{return i.className.indexOf(q)!==-1
}};
var h=function(q){if(l){e.remove(q)
}else{var r,s=i.className;
s=s.split(" ");
for(r=0;
r<s.length;
r++){if(s[r]===q){s[r]=""
...
var j=function(q){if(d){e.toggle(q)
}else{if(f(q)){h(q)
}else{o(q)
}}};
var m=function(){if(k){return e.value
}else{return i.className
}};
return{add:o,contains:f,remove:h,toggle:j,value:m()}
};
return{classAccessor:b,getMethod:c,getObj:a}
}());